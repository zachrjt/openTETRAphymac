...
class LogicalChannelVD():
    ...
    type_5_blocks: NDArray
    type_4_blocks: NDArray
    type_3_blocks: NDArray
    type_2_blocks: NDArray
    type_1_blocks: NDArray
    ...
    def validate_k_length(self, k:int):
        '''
        Validate the number of bits in the K_ type blocks of the logical channel for k=1 or 5
        
        :param self: Description
        :param k: int={1,5}, either verifiy type_1_blocks for k=1, or type_5_blocks for k=5
        '''
        if self.m == 0:
...
class TrafficExplodeMixin:
    ...
    def decimate_multislot_traffic_channel(self:TrafficChannelHost):
        ...
        cls = type(self)
        individual_traffic_channels = []
        for i in range((self.m + self.n - 1)):
            kwargs = {"n": 1}
            individual_traffic_channels.append(cls(**kwargs))
            individual_traffic_channels[-1].type_5_blocks[i] = self.type_5_blocks[i].copy()

        return individual_traffic_channels
...
class TCH_S(TrafficChannel): # pylint: disable=invalid-name
    ...
    def __init__(self, slot_length:str=SlotLength.FULL_SUBSLOT, **kwargs):
        super().__init__(**kwargs)
        self.slot_length = slot_length
        if self.slot_length != SlotLength.HALF_SUBSLOT and self.slot_length != SlotLength.FULL_SUBSLOT:
...
class TCH_4_8(TrafficExplodeMixin, TrafficChannel): # pylint: disable=invalid-name
    '''
...
class TCH_2_4(TrafficExplodeMixin, TrafficChannel): # pylint: disable=invalid-name
    '''
